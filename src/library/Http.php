...
class Http
{
     ...
     * @return bool|string
     */
    public function payload($url, $data = [])
    {
        $options['headers'] = ['Content-Type: application/json'];
        $options['data'] = $data;
        return $this->request('post', $url, $options, true);
    }
     ...
     * @return boolean|string
     */
    public function request($method, $url, $options = [], $pyload = false)
    {
        $curl = curl_init();
        ...
        if (strtolower($method) === 'post') {
            ...
            if ($pyload) {
                ...
                $postdata = $this->buildQueryData($options['data']);
            }
     ...
     * @return array|string
     */
    private function buildQueryData($data, $build = true)
    {
        if (!is_array($data))
            return $data;
     ...
     * @Date 2019/4/29
     */
    public function makeCurlFile(string $file, string $mime = '', string $filename = '')
    {
